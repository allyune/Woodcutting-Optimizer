    ...
    (with-handlers 
        ...
        (when output-path
            (call-with-output-file output-path
                (lambda (output-port)
                    (write-string (cutting-patterns->output cutting-result) output-port))))
        ...
        (define output-csv (cutting-patterns->output cutting-result))
...
(define frame (new frame% [label "Welcome to STAKO cutter"] [width 500] [height 600]))
  ...
  (class canvas%
    (define/override (on-paint)
      (let ([dc (send this get-dc)])
        ...
        (println (send background-color red))
...
; (define explorer-icon (read-bitmap "explorer.png"))
(define dropdown-panel (new my-canvas% [parent frame] [style '(border)]
